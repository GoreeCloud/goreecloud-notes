...
class Note(TimestampMixin, Base):
    ...
    ``content_version`` provides optimistic concurrency for editor saves.
    ``search_vector`` is generated by PostgreSQL from title and string values in
    the structured document so search remains derived from authoritative note
    data instead of becoming a separately mutable client field.
    """
    ...
    search_vector: Mapped[str] = mapped_column(
        TSVECTOR,
        Computed(NOTE_SEARCH_VECTOR_EXPRESSION, persisted=True),
        nullable=False,
    )
