...
1. navigation and account context;
2. note list / Quick Notes capture;
3. focused editor workspace.

The foundation UI is now connected to real authenticated PostgreSQL-backed note persistence. Its current title/body editor is deliberately a simple bridge over the application-owned structured document contract. It is not the final rich-text editor. Rich-text dependency selection, richer block semantics, autosave, and full editor acceptance remain separate gates.
...
The backend owns authentication, authorization, data validation, persistence rules, migration boundaries, export behavior, revision creation, Trash semantics, and attachment authorization.
...
PostgreSQL full-text search remains the preferred search engine. The first workspace API already enforces owner scope before applying filters. The current server-side `q` filter is intentionally basic and is not yet the full-text-search implementation.
...
Authorization is server-side and owner-scoped. Notes and notebooks are fetched through helpers that require `owner_id == authenticated_user.id`. The API deliberately returns the same not-found response for nonexistent objects and objects owned by another user so identifiers are not useful as an ownership-enumeration signal.

## Note Lifecycle and Revision History

Ordinary `DELETE /api/v1/notes/{id}` does not hard-delete a note. It changes native state to `trashed`, preserving recoverability.
